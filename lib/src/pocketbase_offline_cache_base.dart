...
class PbOfflineCache {
  ...
  PbOfflineCache._(this.pb, this.dbIsolate, this.logger,
      [this.indexInstructions = const <String, List<(String name, bool unique, List<String>)>>{},
      this.dbPath,
      this.generateWhereForResync]) {
  ...
  factory PbOfflineCache.withDb(PocketBase pb, CommonDatabase db,
      {Logger? overrideLogger,
      Map<String, List<(String name, bool unique, List<String> columns)>> indexInstructions =
          const <String, List<(String name, bool unique, List<String>)>>{}}) {
    return PbOfflineCache._(pb, DbIsolate.test(db), overrideLogger ?? Logger(), indexInstructions);
  }

  Future<void> createAllIndexesForTable(
      String tableName, Map<String, List<(String, bool, List<String>)>> indexInstructions,
      {Logger? overrideLogger, Set<String>? tableKeys}) async {
    ...
    if (indexesToCreate != null) {
      for (final (String name, bool unique, List<String> columns) entry in indexesToCreate) {
        if (tableKeys != null && !tableKeys.containsAll(entry.$3)) {
          (overrideLogger ?? logger).e(
              "Unable to create index ${entry.$1} on $tableName($tableKeys), could not find all columns: ${entry.$3}");
        } else {
          String columnNames = entry.$3.toString();
          columnNames = columnNames.substring(1, columnNames.length - 1);

          unawaited(dbIsolate.execute(
              "CREATE${entry.$2 ? " UNIQUE" : ""} INDEX IF NOT EXISTS ${entry.$1} ON $tableName($columnNames)"));
        }
      }
    }
  }
  ...
  Future<void> reinitIndexes() async {
    ...
    final List<Map<String, dynamic>> tables =
        await dbIsolate.select("SELECT name FROM sqlite_master WHERE type = 'table'");
  ...
  Future<void> clearOldRecords(int maxOfflineDays) async {
    if (await dbIsolate.enabled()) {
      ...
      for (final Map<String, dynamic> row in data) {
        if (row["name"] != "_last_sync_times" &&
            row["name"] != "sqlite_sequence" &&
            row["name"] != "_operation_queue_params" &&
            row["name"] != "_operation_queue") {
          try {
            unawaited(dbIsolate.execute("DELETE FROM ${row["name"]} WHERE _downloaded < ?",
                <Object>[DateTime.now().toUtc().subtract(Duration(days: maxOfflineDays)).toString()]));
          } catch (e) {}
        }
      }
    }
      ...
      try {
        if (generateWhereForResync != null && await tableExists(dbIsolate, "_last_sync_times")) {
          ...
          for (final Map<String, dynamic> row in syncTimes) {
            final (String, List<Object?>)? whereCondition =
                await generateWhereForResync!(row["table_name"], row["last_update"]);

            if (whereCondition == null) {
              continue;
            }

            final List<Map<String, dynamic>> items = await getRecords(row["table_name"],
                where: whereCondition,
                sort: <(String, bool)>[("updated", false)],
                source: QuerySource.server,
                maxItems: 50);
            if (items.isNotEmpty) {
              ...
              unawaited(dbIsolate.execute(
                  ...
                  <dynamic>[row["table_name"], items.last["updated"]]));
            }
          }
          if (gotNewItems) {
  ...
  Future<void> dequeueCachedOperations() async {
    ...
    final List<Map<String, dynamic>> data =
        await dbIsolate.select("SELECT * FROM _operation_queue ORDER BY created ASC");
    ...
    for (final Map<String, dynamic> operation in data) {
      ...
      final List<Map<String, dynamic>> data =
          await dbIsolate.select("SELECT * FROM _operation_queue_params WHERE operation_id = ?", <String>[operationId]);
  ...
  Future<void> queueOperation(String operationType, String collectionName,
      {Map<String, dynamic>? values, String idToModify = ""}) async {
    if (await dbIsolate.makePort == null) {
      return;
    }
    ...
    final List<Map<String, dynamic>> record = await dbIsolate.select(
        "INSERT INTO _operation_queue (operation_type, created, collection_name, id_to_modify) VALUES ('$operationType', $created, '$collectionName', ?) RETURNING id",
        <Object>[idToModify]);
    ...
    if (values != null) {
      for (final MapEntry<String, dynamic> entry in values.entries) {
        ...
        unawaited(dbIsolate.select(
            "INSERT INTO _operation_queue_params (operation_id, param_key, param_value) VALUES (?, ?, ?)",
            <Object?>[id, entry.key, valueToWrite]));
      }
    }
  }
...
Future<Set<String>> getColumnNames(DbIsolate db, String tableName) async {
  return (await db.select(
    "PRAGMA table_info($tableName)",
      ...
      .map((Map<String, dynamic> col) => col["name"] as String)
      .toSet();
}

(String command, List<dynamic>? params) selectBuilder(
  String tableName, {
  String? columns,
  ...
  (String, List<dynamic> newValues) generateSortCondition(
      Map<String, dynamic>? startAfter, List<(String, bool descending)> sort, bool and, List<dynamic> parameters) {
  ...
  if (filter != null) {
    ...
    final (String whereClause, List<dynamic> items) orderBy =
        generateSortCondition(startAfter, sort, true, <dynamic>[]);
  ...
  if (page != null && page > 1) {
    final int offset = (page - 1) * (maxItems ?? defaultMaxItems);
...
Future<List<Map<String, dynamic>>> getFromLocalDb(
  DbIsolate db,
  String tableName, {
  String? columns,
  (String, List<Object?>)? filter,
  ...
  final (String command, List<dynamic>? params) selectCommand = selectBuilder(
    tableName,
    columns: columns,
    filter: filter,
    maxItems: maxItems,
...
class QueryBuilder {
  ...
  QueryBuilder where(
    ...
    if (isNull == true) {
      return QueryBuilder._(pb, collectionName, "${currentFilter != "" ? "$currentFilter && " : ""}$column = ?",
          List<dynamic>.from(args)..add(null), orderRule, expandFields);
    } else if (isNull == false) {
      return QueryBuilder._(pb, collectionName, "${currentFilter != "" ? "$currentFilter && " : ""}$column != ?",
          List<dynamic>.from(args)..add(null), orderRule, expandFields);
    } else if (isEqualTo != null) {
      return QueryBuilder._(pb, collectionName, "${currentFilter != "" ? "$currentFilter && " : ""}$column = ?",
          List<dynamic>.from(args)..add(isEqualTo), orderRule, expandFields);
    } else if (isNotEqualTo != null) {
      return QueryBuilder._(pb, collectionName, "${currentFilter != "" ? "$currentFilter && " : ""}$column != ?",
          List<dynamic>.from(args)..add(isNotEqualTo), orderRule, expandFields);
    } else if (isGreaterThan != null) {
      return QueryBuilder._(pb, collectionName, "${currentFilter != "" ? "$currentFilter && " : ""}$column > ?",
          List<dynamic>.from(args)..add(isGreaterThan), orderRule, expandFields);
    } else if (isLessThan != null) {
      return QueryBuilder._(pb, collectionName, "${currentFilter != "" ? "$currentFilter && " : ""}$column < ?",
          List<dynamic>.from(args)..add(isLessThan), orderRule, expandFields);
    } else if (isLessThanOrEqualTo != null) {
      return QueryBuilder._(pb, collectionName, "${currentFilter != "" ? "$currentFilter && " : ""}$column <= ?",
          List<dynamic>.from(args)..add(isLessThanOrEqualTo), orderRule, expandFields);
    } else {
      return QueryBuilder._(pb, collectionName, "${currentFilter != "" ? "$currentFilter && " : ""}$column >= ?",
          List<dynamic>.from(args)..add(isGreaterThanOrEqualTo), orderRule, expandFields);
    }
  }

  QueryBuilder expand(List<String> expandFields) {
    return QueryBuilder._(
        pb, collectionName, currentFilter, args, orderRule, List<String>.from(expandFields)..addAll(expandFields));
  }

  QueryBuilder orderBy(String columnName, {bool descending = true}) {
    return QueryBuilder._(pb, collectionName, currentFilter, args,
        List<(String, bool descending)>.from(orderRule)..add((columnName, descending)), expandFields);
  }
  ...
  Future<List<Map<String, dynamic>>> get(
      ...
      Map<String, dynamic>? startAfter}) {
    return pb.getRecords(collectionName,
        where: (currentFilter, args),
        maxItems: maxItems,
        page: page,
        source: source,
        sort: orderRule,
